...
export const createStateApp = createMachine => ({
  /**
  ...
  context(initialContext = {}) {
    ...
    const initialStateMeta = meta[initial] || {}
    initialStateMeta.fields = initialStateMeta.fields || []
    return  mergeContext(context, initialContext);
  },
  meta() {
    const machine = createMachine();
  ...
  createLink( base, id= '', path='') {
    const machine = createMachine();

    const { meta = {}, context = {}, initial  } = machine;
    return  new URL(meta.href(id, path),base).href
